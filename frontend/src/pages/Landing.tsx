...
export default function Landing({ authed }: { authed: boolean }) {
  return (
    <div className="min-h-screen bg-skip-hero">
      <header className="container flex items-center justify-between py-6">
        ...
        <nav className="flex items-center gap-6 text-sm">
          ...
          <Link
            to={authed ? '/app' : '/login'}
            className="rounded-lg bg-primary px-5 py-2 font-semibold text-primary-foreground shadow-glow transition hover:opacity-90"
          >
      ...
      <section className="container relative overflow-hidden py-24 text-center">
        <div className="relative">
          <span className="mb-6 inline-flex items-center gap-2 rounded-full border border-skip-glow bg-card/60 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-primary backdrop-blur">
          ...
          <h1 className="mx-auto max-w-4xl font-display text-5xl font-extrabold leading-tight tracking-tight text-foreground md:text-6xl">
            Do <span className="text-primary">Lovable</span> para o <span className="text-primary">Skip</span>
            <br />
            em um único upload.
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
            ...
            <code className="rounded bg-secondary px-2 py-0.5 font-mono text-sm text-foreground">pnpm build</code> antes de
            devolver o ZIP pronto.
          </p>
          <div className="mt-10 flex items-center justify-center gap-4">
            <Link
              to={authed ? '/app/new' : '/login'}
              className="inline-flex items-center gap-2 rounded-lg bg-primary px-7 py-3.5 font-semibold text-primary-foreground shadow-glow transition hover:translate-y-[-1px] hover:opacity-95"
            >
              Começar migração <ArrowRight className="size-4" />
            </Link>
            <a
              href="https://github.com/PatricSM/skipMigrator"
              className="rounded-lg border border-border bg-card px-7 py-3.5 font-semibold text-foreground transition hover:border-skip-glow hover:bg-secondary"
            >
              Ver no GitHub
            </a>
          </div>
...
function Feature({ icon, title, body }: { icon: React.ReactNode; title: string; body: string }) {
  return (
    <div className="rounded-lg border border-border bg-card p-6 transition hover:border-skip-glow">
      <div className="mb-4 inline-flex items-center justify-center rounded-md bg-primary/15 p-2.5 text-primary">{icon}</div>
...
function Step({ n, title, body }: { n: number; title: string; body: string }) {
  return (
    <li className="flex gap-5 rounded-lg border border-border bg-card p-6 transition hover:border-skip-glow">
      <div className="grid size-10 shrink-0 place-items-center rounded-md bg-primary font-display text-base font-bold text-primary-foreground shadow-glow">
        {n}
      </div>
      <div>
